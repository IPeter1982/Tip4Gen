...
- **Phase 2:** ✅ done (commits `e02496f` + `3763e11` + `370cd43`; provider swapped 2026-06-03, commit `7971f07`). Schema + `IFootballDataProvider` originally fronted api-football; **now backed by worldcup26.ir** (free anonymous JSON, real WC 2026 schedule). `POST /api/admin/fixtures/seed` verified end-to-end against the original WC 2022 dataset (1 tournament, 32 teams, 64 matches, correct stage breakdown). Quota-aware `FixturePoller` in Workers exercised live (`Live → Finished` transition + `MatchFinalized` dispatch). Latent Phase 1 admin-policy bug fixed in passing.
...
- **National-team flags (post-Phase-10):** ✅ done 2026-06-01 (commit `a5b1453`). Country flags appear next to every national-team name (Matches list, TipSubmit header + score-input labels, LiveMatchBanner, AdminMatches table, AdminMatchEditor header, LongTips + AdminLongTips dropdowns). New SPA-only files: `lib/teamFlag.ts` (3-letter team code → ISO 3166-1 alpha-2; built against api-football's non-standard abbreviations `IRA`/`NET`/`SPA`/`JAP`, kept after the 2026-06-03 worldcup26.ir swap since the FIFA-standard aliases were already in the map — harmless redundancy; ENG/WAL/SCO/NIR resolve to flagcdn's `gb-eng`/`gb-wls`/`gb-sct`/`gb-nir` subdivision SVGs), `components/TeamFlag.tsx` (runtime `<img src="https://flagcdn.com/{iso}.svg">` with onError → render-nothing fallback), `components/TeamLabel.tsx` (flag + name flex wrapper used at every render site), `components/TeamSelect.tsx` (`@headlessui/react` Listbox replacing the two native `<select>`s so flags render inside option rows too — RHF integrates via `Controller` in `LongTips`). No backend / schema changes. Topbar menu copy also tweaked: "Hosszú tipp" → "Végső győztes".
- **Dark theme + stadium redesign (post-Phase-10):** ✅ done 2026-06-02. Frontend-only pass. New `web/src/theme/ThemeProvider.tsx` (class-based dark/light with `prefers-color-scheme` fallback + `localStorage['tip4gen.theme']` persistence) and `web/src/components/ThemeToggle.tsx` (Sun/Moon button in Topbar). `web/src/index.css` rewritten as a semantic token system: `@import "tailwindcss"` + `@theme` block defining `--color-surface`/`-elevated`/`-sunken`/`-fg-*`/`-accent*`/`-live*`/`-danger`/`-success`/`-warning`/`-border-*`/`-ring-focus` for the **light** palette, then `.dark { … }` overrides the same names with a **navy/emerald stadium** palette. `@custom-variant dark (&:where(.dark, .dark *));` lets `dark:` work without bumping specificity (only used in `TeamFlag` for the white-flag ring). Custom utilities `@utility glow-accent` (1px ring + 24px soft glow, used on live cards / podium-1 / current-user row) and `@utility live-dot` (1.4s opacity pulse on the live Radio icon). Inline `<head>` script in `web/index.html` sets `.dark` pre-hydration to kill the first-paint flash. `lucide-react` added as a SPA dependency — icons used throughout (Goal, Trophy, Target, Timer, Flame, Star, Shield, Users, Radio, Sun/Moon, etc.). All 36+ files that used `stone-*` / `orange-*` / `red-7..9` / `green-7..` / `yellow-*` raw classes were swept to semantic tokens; `grep "stone-" web/src` is now zero. High-traffic pages got structural reworks: `Home` (gradient hero + DD:HH:MM:SS countdown blocks + 4-card onboarding grid with lucide icons), `Matches` (3-column scoreboard cards with status pills + lucide icons + `glow-accent` on live), `TipSubmit` (hero scoreboard with giant 80×80 score inputs + Star pill joker with `aria-pressed` instead of a checkbox), `Leaderboard` (top-3 podium with `Trophy` medal-coloured icons + 1st-place glow, ranked cards with current-user glow, segmented tab control). Shared components updated: Topbar (`bg-elevated/80 backdrop-blur` + NavLink active state + ThemeToggle slot), Avatar / TeamAvatar (`ring-1` instead of `border-2 border-stone-900`; HSL lightness bumped to 50% so letters are legible on both themes), TeamFlag (`ring-1 ring-border-subtle dark:ring-white/15` — the one legitimate `dark:` usage, for white flags on navy), TeamSelect (token swap + accent-soft focus, `ChevronDown` icon), ConfirmDialog (`bg-elevated rounded-2xl` + `bg-black/60 backdrop-blur-sm` backdrop), LiveMatchBanner (drops orange fill, uses `Radio` icon with `live-dot` pulse + `text-accent` score chip). Admin pages got tokens only — kept `border-2` brutalism intentionally as a visual "admin mode" cue. Token decisions, gotchas, and "things not to do" rules added to `CLAUDE.md` (new "Theme + dark-mode gotchas" section). Build clean (`npm run build --prefix web`), zero backend changes, 225/225 backend tests still green.
- **Manual AI force-run (post-Phase-8):** ✅ done 2026-06-04. New admin endpoint `POST /api/admin/ai-tipper/run/{matchId}` bypasses `AiTipSchedulePolicy` for one match — fires the tipper immediately for every Locked-team AI member that doesn't yet have a tip, writes the 1–1 fallback on any AI failure so the slot never stays empty. Single `admin_audit` row per call (`AdminAuditAction.AiTipperManualRun = 7`) with `{aiMembers, alreadyHadTip}` / `{attempted, written, fallbacks}`. Eligibility = `Scheduled` + kickoff in future (409 `MatchNotEligible` otherwise). SPA: new `AiTipperPanel` in `AdminMatchEditor` between Halasztás and Lemondás, with reason input + Confirm dialog + Hungarian result summary; gating reuses match status. `AiTippingService` constructor grew an `IAdminAuditWriter` dependency.

- **Admin team management (post-Phase-8):** ✅ done 2026-06-07. Closes the "admin can't police teams" gap (only `POST /api/admin/teams/lock` existed before). New `TeamsAdminService` with four operations: list every team (all statuses + roster), rename (`PUT /api/admin/teams/{id}/name`, bypasses `TeamRulesValidator.ValidateMutable` so Locked teams can still be renamed), delete (`DELETE /api/admin/teams/{id}?reason=`, relies on DB `ON DELETE CASCADE` for team_members + team_invites; AI tips/scored_tips cascade via `team_members.id`; human tips/scored_tips persist by design so individual board points stay intact), and remove-member (`DELETE /api/admin/teams/{id}/members/{memberId}?reason=`). Remove-member mirrors `TeamsService.LeaveAsync`'s last-human cascade and adds auto-revert: if the team was `Locked` and now has `< Team.MaxMembers`, calls a new `Team.Unlock()` mutator that flips status back to `Forming` so the existing direct-join path can refill it and `TeamLockJob` can re-lock. No re-scoring call needed — `TeamLeaderboardService` is on-read so the next leaderboard fetch reflects the new roster (under-sized teams get 0 points until refilled). Three new `AdminAuditAction` values (`TeamRenamed = 9`, `TeamDeleted = 10`, `TeamMemberRemoved = 11`); audit rows record minimal `{name}`, `{name, status, memberCount, humanMembers, aiMembers}`, and `{memberId, displayName, isAi, teamName, teamStatus}` / `{remainingMembers, statusAfter, teamCascadeDeleted}` slices respectively. No DB migration (existing FKs do the work). SPA: new `/admin/teams` route + `AdminTeams.tsx` page (brutalist list with rename modal + delete ConfirmDialog + expandable members with per-row remove, contextual Hungarian warnings for "last human → cascade" and "drops below 3 → reverts to Forming"); new nav item ("Csapatok", Users icon); link added on `/admin` landing; `AdminAudit.ACTION_LABEL` extended with the three new Hungarian labels. 220/220 backend tests green (+3 new for `Team.Unlock()` / `Team.Lock()` idempotency).
...
- **Football data source:** **worldcup26.ir** (free, anonymous, real WC 2026 schedule — 104 fixtures + 48 teams) as of 2026-06-03 (commit `7971f07`). Anonymous-first; lazy JWT register-or-login on 401. `local_date` parsed as US Eastern → UTC (sanity-checked against the Mexico City opener); per-stadium IANA TZ map is the fallback if any kickoff drifts. `group_code` populated from day one (no `/standings` enrichment needed). Admin manual-entry (Phase 8) is the contingency if the upstream — a one-maintainer free project — goes offline. The previous api-football Free plan is gone; `WC 2022 season=2022` workaround retired.
...
- [x] Football API account: **worldcup26.ir** — no account needed (anonymous works). `WorldCup26Ir:*` user-secrets only used for the optional JWT register-or-login fallback. The original api-football account is no longer used (was: Free plan, league=1, dev season=2022).
...
- [x] `IFootballDataProvider` in Domain returning `ProviderFixture`/`ProviderTeam` records + normalized `ProviderStatus`. Originally `ApiFootballProvider` (api-football, `x-apisports-key` header). **Now `WorldCup26IrProvider`** — typed `HttpClient` + `AddStandardResilienceHandler`, anonymous-first with a lazy `WorldCup26IrJwtCache` (POSTs `/auth/register` then `/auth/authenticate` on 401, caches the 84-day token).
- [x] `StageMapper` (pure). Originally handled api-football's label formats (`Group A - N`, `Group Stage - N`, `Round of 32`, etc.). **Now `StageMapper.FromWorldCupType(string)`** for worldcup26.ir's lowercase type codes (`"group" | "r32" | "r16" | "qf" | "sf" | "third" | "final"`).
...
- [x] `FixturePoller` (`BackgroundService` in Tip4Gen.Workers) — wakes every `FixturePoller:IntervalMinutes` (default 15), but skips the API call unless DB has a Live match or a Scheduled one within `±ActiveWindowHours / +LookaheadMinutes` (default −4h / +1h). Active-window guard is now defensive insurance (worldcup26.ir's 500 req/min cap is effectively infinite at this cadence) but kept on for cheap robustness. Workers shares Api's `UserSecretsId` so config flows through.
...
- [x] Live verification (api-football era): flipping the Final match to `Live` via psql triggered the next poll tick, transitioned `Live → Finished` with score `2-2`, and dispatched 1 event. The worldcup26.ir provider does not expose a Live status (only `finished: TRUE|FALSE`); the FixturePoller still works because Live transitions happen from our own DB state, not the upstream feed.

**Lessons banked (see commit `3763e11`):**

- (Historical, api-football era) WC fixtures used **matchday-style round labels** (`Group Stage - 1/2/3`); the group letter (A–L) was not in the label, requiring a separate `/standings` enrichment. **Resolved by the 2026-06-03 provider swap** — worldcup26.ir returns the group letter inline on every match, and `Match.GroupCode` is populated during seed.
...
- [x] ~~Group letter enrichment via `/standings`~~ — **resolved 2026-06-03** by the provider swap; worldcup26.ir's `WorldCupGame.group` field carries the letter directly.
...
- [x] Topbar nav: `Mérkőzések`, `Végső győztes` added.
...
**Goal:** 3-person teams form before kickoff, lock at tournament start, score by summing all member points.
...
- The OpenAI prompt currently passes team names as-is. Hungarian names like "Magyarország" can confuse the model (it hallucinated "Curaçao" as the opposing team in one test). Production data from worldcup26.ir uses English names (`name_en`) so the orchestrator path is fine; if you ever localize team names in the DB, pass both English + Hungarian in the prompt.
...
- [x] Infrastructure: `TeamLeaderboardService` — only Locked teams compete (Disqualified members still show on the individual board per §7). Per match: build 3 `MemberPoints` (AI / no-tip = 0), feed `TeamAggregator` (sum of all members, no dropping), accumulate. Tiebreaker = shared placement on tied total (§9's individual-flavoured tiebreakers don't translate; documented inline).
...
- API service deployed from repo root `backend/` via `backend/Dockerfile`. Env: `ASPNETCORE_ENVIRONMENT=Production`, `Auth0__*`, `WorldCup26Ir__BaseUrl` (optional — defaults to `https://worldcup26.ir`) + optional `WorldCup26Ir__AuthEmail`/`__AuthPassword` for the 401 fallback, `OpenAi__ApiKey`, `Cors__AllowedOrigin`. Healthcheck `/api/health`. (Pre-2026-06-03 this used `FootballApi__*` for the api-football integration — delete those keys from the Railway env after the next deploy.)
...
- [x] Onboarding flow on `/` — 4-step checklist (Belépés ✓ · Csapat · Végső győztes · Első tipp) reading `useMyTeam` + `useLongTips` + `useMatches('all')`. Each step has done/locked/pending state; tournament-start countdown banner; guest view shows "Hogyan működik" + login CTA. Dev-only `/api/health` dump removed.
