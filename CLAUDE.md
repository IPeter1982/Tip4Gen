  ...
  src/Tip4Gen.Api           Controllers, Program.cs, Serilog, OpenAPI, Auth0
    ...
    Controllers/            HealthController, MeController (+ avatar PUT/DELETE),
                            ...
                            TeamsController (+ GET / browse all teams +
                              ...
                              PUT /{id}/ai-mode editable after lock),
...
- **AI tipping mode is the one mutation that bypasses `ValidateMutable`.** `TeamsService.SetAiModeAsync` (exposed as `PUT /api/teams/{id}/ai-mode`) only rejects `Disqualified` teams, so captains can re-tune the AI's style after the team locks and the tournament is underway — the new mode takes effect on the next AI tip. Composition (members, name, avatar, invites, leave) still flows through `PatchAsync` / `LeaveAsync` / `AddAiMemberAsync` and stays gated by `ValidateMutable`. The SPA's `AiModePanel` (`web/src/pages/Team.tsx`) computes its own `editable = team.status !== 'Disqualified'` gate; the top-level `editable` in `TeamPanel` is reserved for the composition panels. If you add a new endpoint that should also stay editable post-lock, mirror the SetAiModeAsync shape — don't try to widen `ValidateMutable`, that would silently re-open rename/avatar/leave too.
...
- **Team leaderboard surfaces both `Forming` and `Locked` teams.** `TeamLeaderboardService` filters on `Status != Disqualified` and returns `Status` + `MemberCount` per row so the SPA renders a `Megtelt` / `Csatlakozhatsz · n/3` badge (`Leaderboard.tsx` keys the badge on `memberCount`, not status, because a 3-member `Forming` team pre-tournament is also full). **The scoring path forks on member count, not status**: 3-member teams (Locked OR pre-lock Forming-with-3) go through `TeamAggregator.ForMatch` (the §8 sum-of-3 single source of truth); under-sized teams bypass it (the aggregator enforces `count==3`) and direct-sum the available members' scored-tip points across matches. Same arithmetic, different code path — if the §8 rule ever stops being a plain sum (e.g. dropping a low score), update **both branches** in `TeamLeaderboardService.GetAsync` or they will drift. Ranking is mixed (no Locked-first segregation): all teams sorted by total points descending, then name — a high-scoring `Forming` team can outrank a Locked team and that's intentional.
